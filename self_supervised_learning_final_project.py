...
class MoCoCovLoss(nn.Module):
    ...
    def forward(self, q_raw, k_raw, queue_neg):
        ...
        # =============================================
        # 2. Covariance loss (FIXED)
        #    Use normalized raw features to avoid explosion
        # =============================================
        q_cov = F.normalize(q_raw.detach(), dim=1)
        k_cov = F.normalize(k_raw.detach(), dim=1)

        if self.use_queue_for_cov:
            K = queue_neg.shape[1]
            n = min(1024, K)
            idx = torch.randperm(K, device=q_raw.device)[:n]
            q_extra = queue_neg[:, idx].T  # already normalized keys
            Z = torch.cat([q_cov, k_cov, q_extra], dim=0)
        else:
            Z = torch.cat([q_cov, k_cov], dim=0)
