...
for epoch in range(1, 21):   # <-- update if you train more epochs
...
# ------------------------------------------------------------
# 2. SUN397 Dataset (CSV-based, not ImageFolder)
# ------------------------------------------------------------
...
class SUN397Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not os.path.exists(img_path):           # I added this in the last run to do troubleshoot
            print("MISSING IMAGE:", img_path)      # I added this in the last run to do troubleshoot

        img = Image.open(img_path).convert("RGB")
        img = self.transform(img)

        # train/val have class_id, test does not
        if "class_id" in row:
...
# ------------------------------------------------------------
# 3. Load SUN397 CSV splits
# ------------------------------------------------------------

# SUN397_DIR = "/home/ubuntu/sun397"
...
test_csv  = pd.read_csv(f"{SUN397_DIR}/test_images.csv")    # no labels
...
# ------------------------------------------------------------
# 5. Extract frozen SUN397 features
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 6. Train linear probe on SUN397 train → tune on val
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 7. Train a fresh final classifier using train + val
#    (Recommended for final Kaggle submission)
# ------------------------------------------------------------
...
print("DEBUG LABEL CHECK:")                          # I added these sanity check
print("  Min label:", all_labels.min().item())      # I added these sanity check
print("  Max label:", all_labels.max().item())     # I added these sanity check
print("  Num classes:", num_classes)              # I added these sanity check
...
for epoch in range(10):  # final classifier training is cheap
    ...
    print(f"Final classifier epoch {epoch+1}/10 | Loss: {loss.item():.4f}")


# ------------------------------------------------------------
# 8. Predict on SUN397 test split
# ------------------------------------------------------------
