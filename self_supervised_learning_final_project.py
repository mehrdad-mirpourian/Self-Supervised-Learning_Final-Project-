...
# ================================================================
# FIXED VERSION:
#   - q_raw and k_raw are passed into loss_fn (correct!)
#   - Normalization happens ONLY inside loss_fn for contrastive
#   - Queue receives normalized k_raw (correct MoCo behavior)
# ================================================================
...
for epoch in range(num_epochs):
    ...
    for step, (x1, x2) in enumerate(ssl_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            # 1. RAW embeddings — DO NOT NORMALIZE HERE
            q_raw = model.encoder_q(x1)
            k_raw = model.encoder_k(x2).detach()

            # 2. Compute unified MoCo-Cov loss (now correct!)
            loss, loss_contrast, cov_loss_raw = loss_fn(
                q_raw, k_raw, queue.queue.clone().detach().T
            )
        ...
        # ---------------------------------------------------
        # UPDATE QUEUE (normalize HERE ONLY)
        # ---------------------------------------------------
