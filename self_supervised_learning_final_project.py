...
class MoCoCovLoss(nn.Module):
    def __init__(self, temperature=0.07, lambda_cov=1e-3, use_queue_for_cov=False):
        """
...
class MoCoQueue(nn.Module):
    ...
    def enqueue(self, keys):
        keys = F.normalize(keys, dim=1)   # REQUIRED FIX
...
config = {
    ...
    "lambda_cov": 1e-3,         # FIXED: covariance regularization strength
    "queue_size": 8192,
    "save_every": 1,
...
for epoch in range(num_epochs):
    ...
    for step, (x1, x2) in enumerate(ssl_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss_cov = cov_loss_raw * 1.0  # just renaming
